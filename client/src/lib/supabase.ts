...
// Add a custom setSession method to the supabase.auth object if it doesn't exist
// This helps with compatibility across different Supabase versions
if (supabase && supabase.auth && !supabase.auth.setSession) {
  console.log('Adding custom setSession polyfill to Supabase client');
  
  // Add a setSession method that uses available methods to restore a session
  supabase.auth.setSession = async ({ access_token, refresh_token }: { access_token: string, refresh_token: string }) => {
    try {
      console.log('Using custom setSession polyfill');
      
      // Store the tokens in local storage first using the key format Supabase expects
      const storageKey = 'supabase.auth.token';
      const storageData = JSON.stringify({
        access_token,
        refresh_token,
        expires_at: new Date().getTime() + 3600 * 1000, // Assume 1 hour expiry
      ...
      try {
        ...
        console.log('Tokens stored to storage via polyfill');
      } catch (storageError) {
        console.error('Failed to store tokens in local storage:', storageError);
      }
      ...
      const session = {
        ...
        expires_in: 3600,
        token_type: 'bearer',
        user: null // Need to fetch user separately
      };
      
      // Try to get the user with the access token
      try {
        // Use getUser method which should work with the token we just stored
        const { data: userData, error: userError } = await supabase.auth.getUser(access_token);
        
        if (userError) {
          console.error('Error getting user in setSession polyfill:', userError);
        } else if (userData?.user) {
          console.log('User retrieved in setSession polyfill');
          session.user = userData.user;
        }
      } catch (userError) {
        console.error('Exception getting user in setSession polyfill:', userError);
      }
      
      return { data: { session }, error: null };
    } catch (error) {
      console.error('Error in setSession polyfill:', error);
